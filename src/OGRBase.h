...
//  bool                BeginCreate(OGRwkbType eType);
//  bool                AddPoint(int num,double* x,double* y,double* z = NULL);
//  void                EndCreate();
public:
...
public:
    ...
    bool        CreateField(const char* lpstrFieldName, OGRFieldDataType eType);

    const char* GetName();
public:
    void                Attach(void* pLayer) { m_pOGRLayerAttach = pLayer; }
    void                Detach(){ m_pOGRLayerAttach = NULL; }
    void                Create(void* pLayer);
protected:
...
public:
    ...
    bool    Create(const char* lpstrPath);
    void    Close();

    bool    CreateLayer(COGRLayerBase& layer, const char* lpstrLayerName, OGRwkbType wkbType, const char* lpstrWkt = NULL);
