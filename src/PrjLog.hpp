...
#ifndef _RemoveDir
#define _RemoveDir
inline BOOL RemoveDir(LPCSTR szPath){ char strCmd[1024]; sprintf( strCmd,"rm -f -r \"%s\"",szPath ); system( strCmd );  return TRUE; }
#endif
...
inline char str2chr(const char* str, int sz){
    int a = 0;
    for (int i = 0; i < sz; i++){
        a += *str; str++;
    }
...
inline int CompressStr(const char* src, char* dst, int sz){
    if (sz < 1) return 0;
    int len = strlen(src);  
    if (len < sz){
        strcpy(dst, src);
...
#define ERR_CONF_NOVALUE    0x0025
#define ERR_CONF_VALERR     0x0026  
...
#define ERR_FLOW_FLAG       0x1000
#define ERR_FLOW_PAUSE      0x1001
#define ERR_FLOW_STAT_FALSE 0x1002
#define ERR_FLOW_STAT_TRUE  0x1003
